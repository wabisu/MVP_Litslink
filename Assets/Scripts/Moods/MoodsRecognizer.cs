...
public class MoodsRecognizer : Singleton<MoodsRecognizer>
{
	...
	private IEnumerator SendVerbalData() {
		...
		if (String.IsNullOrEmpty (token)) {
			using (UnityWebRequest www = CreateUnityWebRequest (TOKEN_URL, Encoding.UTF8.GetBytes ("apiKey=" + API_KEY + "&grant_type=client_credentials"))) {
				...
				if (www.isError) {
					...
					token = GetJSONField (JSON_FIELD_TOKEN, www.downloadHandler.text);
		...
		using(UnityWebRequest www = CreateUnityWebRequest (START_URL, Encoding.UTF8.GetBytes("{ dataFormat: { type: \"WAV\" } }") )) {
			...
			if(www.isError) {
				...
				recordingId = GetJSONField (JSON_FIELD_RECORDING_ID, www.downloadHandler.text);
		...
		using (UnityWebRequest www = CreateUnityWebRequest (SEND_DATA_URL + recordingId, WavToBytes.GetByteArray (GetComponent<AudioSource> ().clip))) {
			www.SetRequestHeader ("Authorization", "Bearer " + token);
			yield return www.Send();
			...
			if(www.isError) {
				...
				receivedJSON = new JSONObject (www.downloadHandler.text);
				Debug.Log (receivedJSON.Print(true));

				if (receivedJSON.GetField ("result").GetField ("analysisSegments") == null) {
	...
	private IEnumerator SendWavCoroutine ()
	{
		...
		for (int i = 1; i <= messageLength; i++) {
			...
			Debug.Log ("Recording " + i + " seconds");
			debug.text = "Recording " + i + " seconds";
		}
